...
class DynamicConvolution(nn.Module):
    ...
    def forward(
        ...
        if segment_boundaries is not None:
            ...
            segment_mask = self._create_segment_mask(segment_boundaries, self.kernel_size)
    ...
    def _create_segment_mask(self, boundaries: torch.Tensor, kernel_size: int) -> torch.Tensor:
        ...
        for b, pos in zip(*boundary_positions):
...
class HyenaOperator(nn.Module):
    ...
    def _apply_hyena_recurrence(
        ...
        x_filtered = self._segment_aware_convolution(x1, filter_coeffs, segment_boundaries)
    ...
    def _segment_aware_convolution(
        ...
        if segment_boundaries is not None:
            segment_mask = self._create_causal_segment_mask(segment_boundaries, filter_len)
    ...
    def _create_causal_segment_mask(
        ...
        for b, pos in zip(*boundary_positions):
            ...
            mask[b, start_mask:pos] *= torch.linspace(0.1, 1.0, pos - start_mask, device=boundaries.device)
...
class HyenaFilter(nn.Module):
    ...
    def forward(self, seq_len: int) -> torch.Tensor:
        ...
        sin_components = amplitudes * torch.sin(freqs * t + phases) * torch.exp(decay * t)
        cos_components = amplitudes * torch.cos(freqs * t + phases) * torch.exp(decay * t)
        
        # Combine components
        filter_components = torch.cat([sin_components, cos_components], dim=-1)  # (channels, seq_len, filter_order)
        ...
        filter_coeffs = self.output_proj(filter_components)  # (channels, seq_len, d_model)
