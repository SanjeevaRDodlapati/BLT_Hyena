...
class SegmentAwarePositionalEncoding(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('pe_base', self._create_sinusoidal_encoding(max_len, d_model))
        
        # Learned components for handling merged tokens
        if use_learned_encoding:
            self.segment_encoder = nn.Linear(3, segment_encoding_dim)  # [pos_in_patch, patch_length, global_pos]
            self.position_projection = nn.Linear(d_model + segment_encoding_dim, d_model)
    ...
    def _add_genomic_patterns(self):
        ...
        for period in motif_periods:
            freqs = torch.arange(0, min(self.d_model // 8, 8), 2).float() * (2 * math.pi / period)
    ...
    def forward(
        ...
        segment_encoded = self.segment_encoder(segment_features)  # (batch, seq_len, segment_encoding_dim)
    ...
    def _compute_segment_features(
        ...
        for b in range(batch_size):
            ...
            for i in range(len(boundary_indices) - 1):
                ...
                pos_in_patch = torch.arange(patch_length, device=device, dtype=torch.float)
                ...
                if original_positions is not None:
                    ...
                    global_pos_norm = torch.arange(start_pos, end_pos, device=device, dtype=torch.float) / seq_len
    ...
    def _add_genomic_position_patterns(
        ...
        if hasattr(self, 'codon_freqs') and len(self.codon_freqs) > 0:
            codon_pos = positions.unsqueeze(-1) * self.codon_freqs.unsqueeze(0).unsqueeze(0)
        ...
        if hasattr(self, 'motif_freqs') and len(self.motif_freqs) > 0:
            motif_pos = positions.unsqueeze(-1) * self.motif_freqs.unsqueeze(0).unsqueeze(0)
...
class CrossAttentionPositionBridge(nn.Module):
    ...
    def encode_byte_to_patch(
        ...
        for b in range(batch_size):
            ...
            if len(batch_patches) > 0:
                patch_reprs.append(torch.cat(batch_patches, dim=0))  # (num_patches, d_model)
        ...
        for patches in patch_reprs:
            ...
            if num_patches < max_patches:
                ...
                padding = torch.zeros(pad_size, d_model, device=device, dtype=patches.dtype)
    ...
    def decode_patch_to_byte(
        ...
        target_byte_len: int,
        patch_boundaries: torch.Tensor,
        original_byte_repr: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """
