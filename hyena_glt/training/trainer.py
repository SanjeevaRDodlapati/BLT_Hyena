...
from ..model import HyenaGLT, HyenaGLTForSequenceClassification, HyenaGLTForTokenClassification, HyenaGLTForSequenceGeneration
from .optimization import create_optimizer, create_scheduler, configure_weight_decay
from .multitask import MultiTaskLoss, TaskWeightScheduler, BalancedSampler, TaskConfig
from .curriculum import CurriculumLearning, CurriculumDataLoader
...
class HyenaGLTTrainer:
    ...
    def __init__(
        self,
        model: Union[HyenaGLT, nn.Module],
        ...
        if config.multi_task:
            task_names = list(config.task_weights.keys()) if config.task_weights else ["main"]
    ...
    def _setup_multitask(self):
        ...
        self.multi_task_loss = MultiTaskLoss(
            tasks=tasks,
            weighting_strategy=self.config.task_weighting_strategy
        )
        ...
        if self.config.curriculum_learning:
            self.task_scheduler = TaskWeightScheduler(
                tasks=tasks,
                schedule_type=self.config.curriculum_strategy
            )
    ...
    def _setup_curriculum(self):
        """Setup curriculum learning."""
        from .curriculum import SequenceLengthDifficulty, GenomicComplexityDifficulty
    ...
    def _training_step(self, batch: Dict[str, torch.Tensor]) -> tuple:
        ...
        if self.multi_task_loss and isinstance(outputs, dict):
            ...
            targets = {k.replace('_logits', ''): batch[k.replace('_logits', '_labels')] 
                      for k in predictions.keys() if k.replace('_logits', '_labels') in batch}
    ...
    def evaluate(self, dataloader: Optional[DataLoader] = None) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Evaluating"):
                ...
                if hasattr(outputs, 'loss'):
                    loss = outputs.loss
                    predictions = outputs.logits if hasattr(outputs, 'logits') else outputs.prediction_scores
                else:
                    loss = outputs['loss'] if isinstance(outputs, dict) else outputs[0]
                    predictions = outputs.get('logits', outputs.get('prediction_scores'))
