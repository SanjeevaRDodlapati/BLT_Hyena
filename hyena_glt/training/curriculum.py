...
class SequenceLengthDifficulty(DifficultyMeasure):
    ...
    def __init__(self, normalize: bool = True, max_length: int | None = None):
...
class CurriculumLearning:
    ...
    def compute_example_difficulty(self, example: Any) -> float:
        ...
        weighted_sum = sum(
            d * w for d, w in zip(difficulties, self.difficulty_weights, strict=False)
        )
    ...
    def update_curriculum(self, step: int) -> float:
        ...
        if step >= self.curriculum_steps:
            ...
            if self.curriculum_strategy == "linear":
                self.current_difficulty_threshold = self.start_difficulty + progress * (
                ...
                self.current_difficulty_threshold = self.start_difficulty * np.exp(
                    alpha * progress
                )
                ...
                self.current_difficulty_threshold = (
                    self.start_difficulty + step_idx * step_size
                )
...
class CurriculumDataLoader:
    ...
    def __init__(
        ...
        self.example_difficulties = [
            curriculum.compute_example_difficulty(example) for example in dataset
        ]
    ...
    def get_curriculum_info(self) -> dict[str, Any]:
        ...
        available_examples = sum(
            1
            for difficulty in self.example_difficulties
            if difficulty <= self.curriculum.current_difficulty_threshold
        )
        ...
        return {
            ...
            "utilization": (
                available_examples / total_examples if total_examples > 0 else 0
            ),
