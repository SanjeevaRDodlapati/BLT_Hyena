...
class LayerWiseDecayOptimizer:
    ...
    def _get_layer_wise_param_groups(self) -> list[dict[str, Any]]:
        ...
        for depth, params in depth_params.items():
            ...
            param_groups.append(
                {"params": params, "lr": self.base_lr * lr_scale, "layer_depth": depth}
            )
    ...
    **kwargs,
    ...
    if optimizer_type.lower() == "adamw":
        return torch.optim.AdamW(
            trainable_params, lr=learning_rate, weight_decay=weight_decay, **kwargs
        )
...
def get_parameter_names(
    model: torch.nn.Module, forbidden_layer_types: list[type]
) -> list[str]:
...
def configure_weight_decay(
    ...
    for name, param in model.named_parameters():
        ...
        for module_name, module in model.named_modules():
            if name.startswith(module_name) and isinstance(
                module, tuple(no_decay_types)
            ):
                should_decay = False
                break
