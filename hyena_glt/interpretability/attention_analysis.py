...
class HyenaAttentionAnalyzer:
    ...
    def extract_hyena_patterns(
        ...
        for name, activation in activations.items():
            if len(activation.shape) == 3:  # [batch, seq_len, hidden_size]
                ...
                similarity = torch.matmul(activation_norm, activation_norm.transpose(-2, -1))
    ...
    def analyze_positional_patterns(
        ...
        for layer_name, pattern in attention_patterns.items():
            ...
            for offset in range(1, min(6, seq_len)):  # Check positions ±5
                if offset < seq_len:
                    diagonal_pos = torch.diagonal(pattern, offset=offset, dim1=-2, dim2=-1)
                    diagonal_neg = torch.diagonal(pattern, offset=-offset, dim1=-2, dim2=-1)
                    local_attention += (diagonal_pos.mean() + diagonal_neg.mean()).item()
            ...
            long_range_attention = (pattern * long_range_mask.unsqueeze(0)).sum() / long_range_mask.sum()
    ...
    def detect_attention_motifs(
        ...
        for layer_name, patterns in attention_patterns.items():
            ...
            for batch_idx, (pattern, sequence) in enumerate(zip(patterns, sequences)):
