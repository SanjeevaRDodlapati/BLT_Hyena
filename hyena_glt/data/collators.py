...
class SequenceCollator:
    ...
    def __call__(
        ...
        sequence_lengths = torch.tensor([len(ids) for ids in input_ids], dtype=torch.long)
        ...
        if target_length and self.pad_to_multiple_of:
            target_length = ((target_length + self.pad_to_multiple_of - 1) 
        ...
        if target_length:
            ...
            for ids in input_ids:
                if len(ids) > target_length and self.truncation:
                    ...
                    ids = torch.cat([ids, torch.full((padding_length,), 
                ...
                if target_length:
                    ...
                    for label_seq in labels_list:
                        ...
                        if len(label_seq) > target_length and self.truncation:
                            ...
                            label_seq = torch.cat([label_seq, 
                                                 torch.full((padding_length,), 
                                                          label_pad_id, dtype=torch.long)])
        ...
        if self.include_metadata:
            ...
            for key in features[0].keys():
                if key not in ['input_ids', 'labels', 'attention_mask', 'token_type_ids']:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self.align_lengths and max_length_across_modalities > 0:
            for modality, modal_output in collated_output.items():
                if hasattr(modal_output, 'input_ids') and isinstance(modal_output.input_ids, torch.Tensor):
                    ...
                    if current_length < max_length_across_modalities:
                        ...
                        modal_output.input_ids = torch.cat([modal_output.input_ids, padding], dim=1)
    ...
    def create_adaptive_batches(
        ...
        for i, (feature, length) in enumerate(zip(features, lengths)):
            ...
            should_start_new_batch = (
                len(current_batch) >= self.max_sequences_per_batch or
                proposed_tokens > self.target_tokens_per_batch or
                (current_batch and 
                 abs(length - current_max_length) / max(length, current_max_length) > self.length_tolerance)
            )
            
            if should_start_new_batch and current_batch:
                batches.append(current_batch)
                current_batch = [feature]
                current_batch_tokens = length
                current_max_length = length
            else:
                current_batch.append(feature)
                current_batch_tokens = proposed_tokens
...
class StreamingCollator:
    ...
    def add_to_buffer(self, feature: Dict[str, Any]) -> Optional[GenomicCollatorOutput]:
        """
    ...
    def flush_buffer(self) -> Optional[GenomicCollatorOutput]:
        """
