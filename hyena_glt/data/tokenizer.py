...
class GenomicTokenizer(PreTrainedTokenizer):
    ...
    def _tokenize(self, text: str) -> list[str]:
        ...
        for i in range(
            0, len(sequence) - self.kmer_size + 1, self.kmer_size - self.overlap_size
        ):
        ...
        remaining_start = len(sequence) - (len(sequence) - self.kmer_size) % (
            self.kmer_size - self.overlap_size
        )
        ...
        **kwargs: Any,
        ...
        output = {
            ...
            "attention_mask": (
                ...
                if len(all_attention_masks) == 1
                else all_attention_masks
            ),
        }
        ...
        if return_tensors == "pt":
            ...
            for key, value in output.items():
                if isinstance(value, list) and value and isinstance(value[0], list):
                    tensor_output[key] = torch.tensor(value)
                else:
                    tensor_output[key] = torch.tensor(
    ...
    def decode(
        ...
        if skip_special_tokens:
            tokens = [
                token for token in tokens if token not in self.special_tokens.values()
            ]
...
class RNATokenizer(GenomicTokenizer):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        kwargs["enable_reverse_complement"] = kwargs.get(
...
class ProteinTokenizer(GenomicTokenizer):
    ...
    def encode_with_properties(
        self, sequence: str, include_properties: bool = True, **kwargs: Any
    ) -> dict[str, Any]:
