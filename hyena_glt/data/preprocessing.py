...
class GenomicPreprocessor:
    ...
    def preprocess_sequences(
        self,
        sequences: list[str | SeqRecord],
        qualities: list[list[float] | None] | None = None,
        ...
        if qualities is None:
            seq_qualities: list[list[float] | None] = [None] * len(sequences)
        else:
            seq_qualities = qualities
        ...
        for i, seq in enumerate(sequences):
            if isinstance(seq, SeqRecord):
                ...
                seq_headers.append(
                    headers[i] if headers and i < len(headers) else f"seq_{i}"
                )
        ...
        processed_sequences = []
        processed_headers = []
        seen_sequences: set[str] | None = set() if self.remove_duplicates else None

        for seq, header, quality in zip(
            seq_strings, seq_headers, seq_qualities, strict=False
        ):
            ...
            if self.remove_duplicates and seen_sequences is not None:
    ...
    def _preprocess_single_sequence(
        self, sequence: str, quality: list[float] | None = None
    ) -> str | None:
    ...
    def _preprocess_protein(self, sequence: str) -> str:
        """Protein-specific preprocessing."""
        # Replace ambiguous amino acids with X
        sequence = re.sub(r"[^ACDEFGHIKLMNPQRSTVWY]", "X", sequence)
        return sequence

    def _filter_by_quality(self, sequence: str, quality: list[float]) -> str:
        """Filter sequence based on quality scores."""
        if len(quality) != len(sequence):
            warnings.warn(
                "Quality and sequence length mismatch, skipping quality filtering",
                stacklevel=2,
            )
            return sequence
        ...
        filtered_chars = []
        for char, qual in zip(sequence, quality, strict=False):
    ...
    def preprocess_file(
        ...
        processed_seqs, processed_headers, stats = self.preprocess_sequences(
            cast(list[str | SeqRecord], sequences), qualities, headers
        )

        # Save if output path provided
        if output_path:
            self._save_sequences(
                processed_seqs, processed_headers, output_path, file_format
            )
    ...
    def _load_sequences(
        ...
        sequences = []
        headers = []
        qualities: list[list[float] | None] = []
        ...
        try:
            ...
            warnings.warn(f"Error loading file {file_path}: {e}", stacklevel=2)
    ...
    def _save_sequences(
        ...
        records = []
        for seq, header in zip(sequences, headers, strict=False):
        ...
        try:
            ...
            warnings.warn(
                f"Error saving with BioPython: {e}. Falling back to simple format.",
                stacklevel=2,
            )
...
class SequenceAugmenter:
    ...
    def _random_deletion(self, sequence: str) -> str:
        ...
        sequence_list = list(sequence)
        num_deletions = max(
            1, min(len(sequence) // 2, int(len(sequence) * self.augmentation_ratio))
        )
        ...
        for _ in range(num_mutations):
            ...
            if original_char in conservative_groups:
                conservative_options = [
                    c for c in conservative_groups[original_char] if c != original_char
                ]
...
class MotifExtractor:
    ...
    def __init__(self, sequence_type: str = "dna"):
        ...
        self.known_motifs: dict[str, dict[str, str | list[str]]] = {
            "dna": {
                "TATA_box": "TATAAA",
                "CAAT_box": "CAAT",
                "GC_box": "GGCC",
    ...
    def extract_motifs(
        self, sequences: list[str], motif_length: int = 6, min_occurrences: int = 2
    ) -> dict[str, int]:
        """
    ...
    def find_known_motifs(self, sequence: str) -> dict[str, list[tuple[int, str]]]:
        ...
        for motif_name, motif_pattern in known.items():
            if isinstance(motif_pattern, list):
                ...
                # Single pattern
                found_motifs[motif_name] = self._find_pattern_matches(
...
class QualityController:
    ...
    def analyze_sequence_quality(
        sequences: list[str], sequence_type: str = "dna"
    ) -> dict[str, Any]:
        """
        ...
        if sequence_type in ["dna", "rna"]:
            ...
            analysis["composition"] = {
                ...
                "T" if sequence_type == "dna" else "U": (
                    all_chars.count("T" if sequence_type == "dna" else "U")
                    / total_chars
                    if total_chars > 0
                    else 0
                ),
                ...
                "GC_content": (
                    (all_chars.count("G") + all_chars.count("C")) / total_chars
                    if total_chars > 0
                    else 0
                ),
            }
    ...
    def filter_sequences_by_quality(
        ...
        filtered = []
        seen: set[str] | None = set() if remove_duplicates else None
        ...
        set(valid_chars.get(sequence_type, ""))
        ...
        for seq in sequences:
            ...
            # Duplicate filter
            if remove_duplicates and seen is not None:
                if seq in seen:
