...
class MemoryOptimizer:
    ...
    def _apply_activation_checkpointing(self, model: HyenaGLT) -> nn.Module:
        ...
        logger.info(f"Applied activation checkpointing to {len(self.config.checkpoint_activations)} modules")
        return model
    ...
    def _get_checkpointable_layers(self, model: nn.Module) -> List[Tuple[str, nn.Module]]:
    ...
    def _is_checkpointable(self, module: nn.Module) -> bool:
        ...
        return isinstance(module, checkpointable_types) or 'block' in type(module).__name__.lower()
...
class MemoryBenchmark:
    ...
    def print_benchmark_results(self):
        ...
        for config_name, stats in self.benchmark_results.items():
            ...
            print(f"  Memory Range: {stats['min_memory']:.2f} - {stats['max_memory']:.2f} MB")
        ...
        if len(self.benchmark_results) > 1:
            ...
            for config_name, stats in list(self.benchmark_results.items())[1:]:
                savings = (baseline_memory - stats['avg_memory']) / baseline_memory * 100
                print(f"  {config_name} vs {baseline_config}: {savings:.1f}% memory savings")
