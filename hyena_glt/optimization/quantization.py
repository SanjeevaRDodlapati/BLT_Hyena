...
class QATTrainer:
    ...
    def train(
        ...
        for epoch in range(self.config.qat_epochs):
            ...
            logger.info(
                f"QAT Epoch {epoch+1}/{self.config.qat_epochs}, Loss: {avg_loss:.4f}"
            )
    ...
    def calibrate(
        ...
        with torch.no_grad():
            for batch_idx, (data, _) in enumerate(calibration_loader):
                ...
                if batch_idx % 100 == 0:
                    logger.info(
                        f"Calibration progress: {samples_processed}/{self.config.calibration_samples}"
                    )
...
class QuantizationBenchmark:
    ...
    def benchmark(
        ...
        if "accuracy" in metrics:
            results["original"]["accuracy"] = self._measure_accuracy(
                original_model, test_loader
            )
        if "latency" in metrics:
            results["original"]["latency"] = self._measure_latency(
                original_model, test_loader
            )
        ...
        if "accuracy" in metrics:
            results["quantized"]["accuracy"] = self._measure_accuracy(
                quantized_model, test_loader
            )
        if "latency" in metrics:
            results["quantized"]["latency"] = self._measure_latency(
                quantized_model, test_loader
            )
        if "memory" in metrics:
    ...
    def print_results(self):
        ...
        if (
            "accuracy" in self.results["original"]
            and "accuracy" in self.results["quantized"]
        ):
            accuracy_drop = (
                self.results["original"]["accuracy"]
                - self.results["quantized"]["accuracy"]
            )
