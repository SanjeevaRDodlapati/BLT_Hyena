...
class ModelPruner:
    ...
    def _magnitude_prune(self, model: HyenaGLT) -> nn.Module:
        ...
        if self.config.global_pruning:
            ...
            for name, module in model_copy.named_modules():
                if isinstance(module, (nn.Linear, nn.Conv1d, nn.Conv2d)):
                    if name not in self.config.skip_layers:
                        sparsity = self.config.layer_sparsities.get(name, self.config.sparsity)
    ...
    def _gradient_prune(
        ...
        importance_scores = gradient_pruner.compute_importance_scores(model, train_loader)
        ...
        if self.config.global_pruning:
            ...
            for name, module in model_copy.named_modules():
                if isinstance(module, (nn.Linear, nn.Conv1d, nn.Conv2d)):
                    if name not in self.config.skip_layers and name in importance_scores:
                        scores = importance_scores[name].flatten()
                        all_scores.extend(scores.tolist())
                        param_mapping.extend([(module, 'weight', i) for i in range(len(scores))])
    ...
    def _random_prune(self, model: HyenaGLT) -> nn.Module:
        ...
        for name, module in model_copy.named_modules():
            if isinstance(module, (nn.Linear, nn.Conv1d, nn.Conv2d)):
                if name not in self.config.skip_layers:
                    prune.random_unstructured(module, name='weight', amount=self.config.sparsity)
    ...
    def _finetune_pruned_model(
        ...
        for epoch in range(self.config.finetune_epochs):
            ...
            logger.info(f"Fine-tuning Epoch {epoch+1}/{self.config.finetune_epochs}, Loss: {avg_loss:.4f}")
    ...
    def get_sparsity_info(self, model: nn.Module) -> Dict[str, Any]:
        ...
        sparsity_info['overall_sparsity'] = pruned_params / total_params if total_params > 0 else 0.0
...
class StructuredPruner:
    ...
    def _prune_conv(self, module: Union[nn.Conv1d, nn.Conv2d]):
    ...
    def _prune_conv_filters(self, module: Union[nn.Conv1d, nn.Conv2d]):
    ...
    def _prune_conv_channels(self, module: Union[nn.Conv1d, nn.Conv2d]):
        """Prune input channels in convolutional layer."""
        weight = module.weight
        
        # Calculate importance scores for each input channel
        importance = torch.norm(weight.view(weight.size(0), weight.size(1), -1), dim=(0, 2))
...
class PruningScheduler:
    ...
    def get_current_sparsity(self) -> float:
        ...
        sparsity = (
            ...
            progress * (self.config.final_sparsity - self.config.initial_sparsity)
        )
