...
def gpu_memory_usage() -> dict[str, float] | None:
    ...
    for i in range(torch.cuda.device_count()):
        device_stats = {
            ...
            f"device_{i}_max_allocated_mb": torch.cuda.max_memory_allocated(i)
            / 1024
            / 1024,
        }
...
def benchmark_model(
    model_fn: Callable, input_data: Any, num_runs: int = 10, warmup_runs: int = 2
) -> dict[str, float]:
    ...
    logger.info(
        f"Benchmark completed: avg={avg_time:.4f}s, min={min_time:.4f}s, max={max_time:.4f}s"
    )
    return stats
...
def measure_throughput(
    ...
    logger.info(
        f"Throughput: {stats['iterations_per_second']:.2f} iter/s, {stats['items_per_second']:.2f} items/s"
    )
    return stats
