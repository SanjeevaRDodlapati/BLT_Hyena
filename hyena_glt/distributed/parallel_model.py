...
class DistributedHyenaGLT(nn.Module):
    ...
    def get_model_size(self) -> Dict[str, Any]:
        ...
        trainable_params = sum(p.numel() for p in self._original_model.parameters() if p.requires_grad)
...
class ModelParallelHyenaGLT(nn.Module):
    ...
    def __init__(
        ...
        model: nn.Module,
        device_manager: DeviceManager,
        split_layers: Optional[List[str]] = None,
    ):
        """
        ...
        if device_manager.device_count < 2:
            logger.warning("Model parallelism requires multiple GPUs, falling back to single GPU")
