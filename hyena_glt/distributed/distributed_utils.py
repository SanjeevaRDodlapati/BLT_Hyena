...
def all_reduce_tensor(tensor: torch.Tensor, device_manager: DeviceManager) -> torch.Tensor:
    """
...
def gather_tensor(tensor: torch.Tensor, device_manager: DeviceManager) -> Optional[torch.Tensor]:
    """
    ...
    if device_manager.is_main_process:
        gather_list = [torch.zeros_like(tensor) for _ in range(device_manager.world_size)]
