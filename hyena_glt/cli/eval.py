...
Examples:
  ...
  hyena-glt-eval --config eval_config.json --output results/
    ...
    model_group.add_argument("--model", type=str, help="Path to saved model file (.pt)")
    model_group.add_argument(
        "--checkpoint", type=str, help="Path to training checkpoint"
    )
    ...
    parser.add_argument(
        "--config", type=str, help="Path to evaluation configuration JSON file"
    )
    ...
    eval_group.add_argument(
        "--metrics", type=str, help="Comma-separated list of metrics to compute"
    )
    ...
    output_group.add_argument(
        "--save-embeddings", action="store_true", help="Save sequence embeddings"
    )
    output_group.add_argument(
...
def save_results(
    results: dict[str, Any], output_dir: str, format: str = "json"
) -> None:
