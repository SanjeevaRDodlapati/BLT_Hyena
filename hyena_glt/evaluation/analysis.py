...
class ModelAnalyzer:
    ...
    def analyze_attention_patterns(self, input_ids: torch.Tensor, 
        ...
        with torch.no_grad():
            ...
            for layer_idx in layer_indices:
                if layer_idx < len(attentions):
                    ...
                    attn_weights = np.mean(attn_weights, axis=(0, 1))  # Average batch and heads
                    attention_patterns[f'layer_{layer_idx}'] = attn_weights
        
        return attention_patterns
    
    def analyze_hidden_representations(self, input_ids: torch.Tensor, 
                                     layer_indices: Optional[List[int]] = None) -> Dict[str, np.ndarray]:
        """
        ...
        for name, activation in activations.items():
            if layer_indices is None or any(f'layer_{i}' in name or f'block_{i}' in name for i in layer_indices):
                # Average across sequence length
                hidden_states[name] = activation.mean(dim=1).cpu().numpy()
        
        return hidden_states
    
    def compute_feature_importance(self, input_ids: torch.Tensor, 
...
class ResultsAnalyzer:
    ...
    def plot_metrics_comparison(self, metrics: List[str], 
        ...
        plot_data = summary_df[['task'] + [m for m in metrics if m in summary_df.columns]]
    ...
    def analyze_task_difficulty(self) -> Dict[str, float]:
        ...
        positive_metrics = ['accuracy', 'f1', 'precision', 'recall', 'auc_roc', 'auc_pr']
        ...
        for _, row in summary_df.iterrows():
            ...
            for metric in positive_metrics:
                if metric in row and not pd.isna(row[metric]):
                    ...
                    max_val = summary_df[metric].max() if summary_df[metric].max() > 0 else 1
...
class StatisticalAnalyzer:
    ...
    def compute_significance_tests(self, metric: str, 
                                 comparison_pairs: Optional[List[Tuple[str, str]]] = None) -> Dict[str, Dict]:
        """
        ...
        if comparison_pairs is None:
            ...
            comparison_pairs = [(tasks[i], tasks[j]) for i in range(len(tasks)) 
    ...
    def compute_confidence_intervals(self, metric: str, confidence: float = 0.95) -> Dict[str, Tuple[float, float]]:
        """
...
class VisualizationUtils:
    ...
    @staticmethod
    def create_interactive_performance_dashboard(results: Dict[str, EvaluationResult],
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=['Accuracy Metrics', 'F1 Scores', 'Precision vs Recall', 'Metric Distribution'],
        ...
        if 'accuracy' in df.columns:
            fig.add_trace(
                go.Bar(x=df['Task'], y=df['accuracy'], name='Accuracy', marker_color='blue'),
                row=1, col=1
            )
        ...
        if 'precision' in df.columns and 'recall' in df.columns:
            fig.add_trace(
                go.Scatter(x=df['precision'], y=df['recall'], mode='markers+text',
                          text=df['Task'], textposition='top center',
    ...
    def plot_learning_curves(training_history: Dict[str, List[float]],
        ...
        for i, metric in enumerate(metrics):
            ...
            ax.plot(epochs, training_history[metric], label=f'Training {metric}', marker='o')
            ax.plot(epochs, validation_history[metric], label=f'Validation {metric}', marker='s')
...
def create_comprehensive_report(results: Dict[str, EvaluationResult],
                              output_dir: str,
                              model_name: str = "HyenaGLT") -> str:
    """
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"This report presents comprehensive evaluation results for the {model_name} model ")
        ...
        f.write("- Implement transfer learning from high-performing to low-performing tasks\n\n")
