...
class EvaluationVisualizer:
    ...
    def plot_task_performance_comparison(
        ...
        for i, metric in enumerate(metrics):
            ...
            ax1.bar(
                x + i * width, values, width, label=metric, color=colors[i], alpha=0.8
            )
    ...
    def plot_scalability_analysis(
        self, scalability_data: dict[str, Any], save_path: str | None = None
    ) -> plt.Figure:
        """
        ...
        if "batch_size_scaling" in scalability_data:
            ...
            if "metrics" in batch_data:
                ...
                for metric in batch_data["metrics"]:
                    if "error" not in metric:
                        ...
                        inference_times.append(
                            metric.get("speed_metrics", {}).get(
                                "mean_inference_time", 0
                            )
                        )
                        memory_usage.append(
                            metric.get("memory_metrics", {}).get("peak_memory_mb", 0)
                        )
                ...
                line2 = ax1_twin.plot(
                    batch_sizes, memory_usage, "ro-", label="Memory Usage", linewidth=2
                )
        ...
        if "sequence_length_scaling" in scalability_data:
            ...
            if "metrics" in seq_data:
                ...
                line2 = ax2_twin.plot(
                    seq_lengths, memory_usage, "mo-", label="Memory Usage", linewidth=2
                )
        ...
        if "batch_size_scaling" in scalability_data:
            ...
            if "metrics" in batch_data:
                ...
                for metric in batch_data["metrics"]:
                    if "error" not in metric:
                        batch_sizes.append(metric["batch_size"])
                        throughput = metric.get("speed_metrics", {}).get(
    ...
    def plot_training_curves(
        ...
        validation_history: dict[str, list[float]] | None = None,
        save_path: str | None = None,
    ) -> plt.Figure:
        """
        ...
        for i, metric in enumerate(metrics):
            ...
            if validation_history and metric in validation_history:
                best_val = (
                    max(validation_history[metric])
                    if "acc" in metric or "f1" in metric
                    else min(validation_history[metric])
                )
    ...
    def plot_confusion_matrices(
        ...
        for i, (task_name, cm) in enumerate(confusion_matrices.items()):
            ...
            if class_names and task_name in class_names:
                ...
                ax.set_yticks(tick_marks)
                ax.set_xticklabels(
                    [f"Class {i}" for i in range(n_classes)], rotation=45
                )
            ...
            for i_cm in range(cm.shape[0]):
                for j_cm in range(cm.shape[1]):
                    ax.text(
                        ...
                        va="center",
                        color=(
                            "white" if cm_normalized[i_cm, j_cm] > thresh else "black"
                        ),
                        fontsize=9,
                    )
...
def create_evaluation_dashboard(
    results: dict[str, Any], output_dir: str, model_name: str = "HyenaGLT"
) -> list[str]:
    """
    ...
    if scalability_data:
        ...
        fig = visualizer.plot_scalability_analysis(
            scalability_data, str(scalability_path)
        )
        plt.close(fig)
