...
class PerformanceBenchmark:
    ...
    def generate_synthetic_data(self, num_samples: int, seq_length: int) -> Tuple[List[str], List[int]]:
    ...
    def measure_memory_usage(self) -> Dict[str, float]:
        ...
        return {
            ...
            "ram_peak_gb": memory_info.peak_wss / 1024**3 if hasattr(memory_info, 'peak_wss') else None,
            **gpu_memory
        }
    ...
    def benchmark_training(
        ...
        self.logger.info(f"Benchmarking: {model_size}, seq_len={seq_length}, batch={batch_size}, data={dataset_size}")
    ...
    def _estimate_memory_usage(self, model_size: str, seq_length: int, batch_size: int) -> float:
    ...
    def generate_report(self, results_df: pd.DataFrame):
        ...
        report = {
            ...
            "performance_stats": {
                ...
                "max_memory_usage_gb": successful_runs.apply(
                    lambda row: max(row['memory_after'].values()) if row['memory_after'] else 0, 
                    axis=1
                ).max()
            },
            "best_configurations": {
                ...
                ][['model_size', 'sequence_length', 'batch_size', 'samples_per_second']].to_dict(),
                
                "most_memory_efficient": successful_runs.loc[
                    successful_runs.apply(
                        lambda row: max(row['memory_after'].values()) if row['memory_after'] else float('inf'), 
                        axis=1
                    ).idxmin()
        ...
        print(f"Max samples/second: {report['performance_stats']['max_samples_per_second']:.2f}")
        print(f"Max memory usage: {report['performance_stats']['max_memory_usage_gb']:.2f} GB")
...
def main():
    ...
    parser.add_argument("--model-sizes", nargs="+", default=["tiny", "small"], 
    ...
    parser.add_argument("--dataset-sizes", nargs="+", type=int, default=[100, 500],
