...
from hyena_glt.model.position_embeddings import SegmentAwarePositionalEncoding, CrossAttentionPositionBridge
...
def demo_entropy_based_patching():
    ...
    low_entropy_seq = torch.tensor([1, 2, 3, 4] * (seq_len // 4), dtype=torch.long).unsqueeze(0)
    ...
    if outputs.get('merge_info'):
        for i, merge_info in enumerate(outputs['merge_info']):
            print(f"Layer {i+1} compression ratio: {merge_info['compression_ratio']:.2f}")
...
def demo_cross_attention_bridges():
    ...
    cosine_sim = torch.cosine_similarity(
        reconstructed.view(-1, d_model), 
        byte_repr.view(-1, d_model), 
        dim=1
    ).mean()
    
    print(f"Reconstruction MSE: {mse.item():.6f}")
    print(f"Average cosine similarity: {cosine_sim.item():.4f}")
    print("✓ Cross-attention bridges preserve position information through U-shape flow")
...
def demo_position_embedding_preservation():
    ...
    position_correlation = torch.corrcoef(torch.stack([
        merged_encoded.view(-1),
        reconstructed.view(-1)
    ]))[0, 1]
    
    print(f"Position correlation after reconstruction: {position_correlation.item():.4f}")
...
def demo_genomic_position_patterns():
    ...
    for pattern_name, length in genomic_patterns.items():
        ...
        print(f"  {pattern_name} (length {length}): pattern effect = {pattern_effect.item():.4f}")
    
    print("✓ Genomic-specific patterns enhance position encoding for biological sequences")
