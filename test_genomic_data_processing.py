...
class GenomicSequenceGenerator:
    ...
    def generate_genomic_variants(self, reference_seq: str, num_variants: int = 5) -> List[Tuple[str, str, int]]:
...
class GenomicDatasetTest:
    ...
    def test_promoter_sequence_processing(self):
        ...
        promoter_seqs = [
            self.seq_generator.generate_promoter_region(length=1000)
            for _ in range(10)
        ]
        ...
        for i, seq in enumerate(promoter_seqs):
            ...
            with torch.no_grad():
                ...
                embeddings = torch.randn(1, len(tokens), self.config.hidden_size).to(self.device)
                
                # Test position encoding
                pos_embeddings = self.position_manager.encode_positions(
                    embeddings, 
                    original_positions=torch.arange(len(tokens)).unsqueeze(0).to(self.device)
                )
    ...
    def test_coding_sequence_processing(self):
        ...
        for length in lengths:
            ...
            print(f"   📏 Length {length}: {avg_tokens:.1f} tokens, {avg_time*1000:.2f}ms avg")
    ...
    def test_variant_effect_analysis(self):
        ...
        # Generate variants
        variants = self.seq_generator.generate_genomic_variants(reference_seq, num_variants=10)
        ...
        with torch.no_grad():
            ref_embeddings = torch.randn(1, len(ref_tokens), self.config.hidden_size).to(self.device)
            ref_pos_embeddings = self.position_manager.encode_positions(
                ref_embeddings,
                original_positions=torch.arange(len(ref_tokens)).unsqueeze(0).to(self.device)
            )
        ...
        for variant_seq, variant_type, position in variants:
            ...
            with torch.no_grad():
                var_embeddings = torch.randn(1, len(var_tokens), self.config.hidden_size).to(self.device)
                var_pos_embeddings = self.position_manager.encode_positions(
                    var_embeddings,
                    original_positions=torch.arange(len(var_tokens)).unsqueeze(0).to(self.device)
                )
            ...
            if var_pos_embeddings.shape[1] == ref_pos_embeddings.shape[1]:
                embedding_diff = torch.mean(torch.abs(var_pos_embeddings - ref_pos_embeddings))
            else:
        ...
        # Analysis
        snp_results = [r for r in variant_results if r['variant_type'].startswith('SNP')]
        ins_results = [r for r in variant_results if r['variant_type'].startswith('INS')]
        del_results = [r for r in variant_results if r['variant_type'].startswith('DEL')]
        
        print(f"   📊 Processed {len(variants)} variants:")
        if snp_results:
            avg_snp_diff = np.mean([r['embedding_difference'] for r in snp_results if r['embedding_difference'] != float('inf')])
            print(f"      🔸 SNPs: {len(snp_results)} variants, avg embedding diff: {avg_snp_diff:.4f}")
        
        if ins_results:
            avg_ins_diff = np.mean([r['embedding_difference'] for r in ins_results if r['embedding_difference'] != float('inf')])
            print(f"      🔸 Insertions: {len(ins_results)} variants, avg embedding diff: {avg_ins_diff:.4f}")
        
        if del_results:
            avg_del_diff = np.mean([r['embedding_difference'] for r in del_results if r['embedding_difference'] != float('inf')])
            print(f"      🔸 Deletions: {len(del_results)} variants, avg embedding diff: {avg_del_diff:.4f}")
    ...
    def test_long_sequence_processing(self):
        ...
        for length in lengths:
            ...
            results.append({
                ...
                'processing_time': processing_time,
                'success': success,
                'tokens_per_second': len(tokens) / processing_time if success else 0
            })
            
            if success:
                print(f"      ✅ Processed {len(tokens)} tokens in {processing_time*1000:.1f}ms")
                print(f"      🚀 Throughput: {len(tokens)/processing_time:.0f} tokens/sec")
    ...
    def test_position_preservation_across_tasks(self):
        ...
        for seq_type, sequence in sequences.items():
            ...
            results[seq_type] = {
                ...
                'bridge_shape': bridge_output.shape if bridge_output is not None else None
            }
    ...
    def run_comprehensive_test(self):
        ...
        if 'long_sequence_processing' in self.results:
            successful_long = sum(1 for r in self.results['long_sequence_processing'] if r['success'])
            total_long = len(self.results['long_sequence_processing'])
            print(f"📏 Long sequence processing: {successful_long}/{total_long} lengths successful")
        ...
        if 'position_preservation' in self.results:
            bridge_success = sum(1 for r in self.results['position_preservation'].values() if r['bridge_success'])
            total_seq_types = len(self.results['position_preservation'])
            print(f"🎯 Position preservation: {bridge_success}/{total_seq_types} sequence types successful")
        
        print("\n🎉 Genomic data testing complete!")
        
        # Save results
        torch.save(self.results, '/Users/sanjeev/Downloads/Repos/BLT_Hyena/genomic_test_results.pt')
