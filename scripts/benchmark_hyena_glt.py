...
def create_sample_genomic_datasets(num_samples_per_task: int = 1000) -> Dict[str, Any]:
    ...
    for i in range(num_samples_per_task):
        ...
        has_tata = 'TATAAA' in sequence or 'TATAWA' in sequence.replace('T', 'W').replace('A', 'W')
    ...
    logger.info(f"Created {len(datasets)} datasets with {num_samples_per_task} samples each")
    return datasets
...
def run_comprehensive_benchmark(config_path: Optional[str] = None,
                              output_dir: str = "./benchmark_results",
                              quick_mode: bool = False) -> Dict[str, Any]:
    """Run comprehensive benchmarking suite."""
    
    from hyena_glt.evaluation.benchmark import (
        BenchmarkConfig, BenchmarkRunner, create_genomic_benchmark_suite
    )
    from hyena_glt.evaluation.analysis import create_comprehensive_report
    ...
    if config_path and Path(config_path).exists():
        ...
        if quick_mode:
            ...
            for config in benchmark_configs:
                ...
                config.sequence_lengths = [512, 1024] if config.sequence_lengths else None
...
def create_benchmark_data_loaders(datasets: Dict[str, Any], 
...
def generate_benchmark_summary(results: Dict[str, Any], output_dir: str) -> str:
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write("## Overview\n\n")
        f.write(f"This report summarizes the benchmarking results for {len(results)} model variants ")
        ...
        for model_name, model_results in results.items():
            ...
            for benchmark_name, benchmark_result in model_results.items():
                if hasattr(benchmark_result, 'results') and 'parameter_analysis' in benchmark_result.results:
                    param_info = benchmark_result.results['parameter_analysis']
                    break
            
            if param_info:
                f.write(f"- **Total Parameters**: {param_info.get('total_parameters', 'Unknown'):,}\n")
                f.write(f"- **Trainable Parameters**: {param_info.get('trainable_parameters', 'Unknown'):,}\n")
                    ...
                    for key, value in benchmark_result.results.items():
                        if key.startswith('dataset_'):
                            ...
                            if 'summary_metrics' in value:
                                task_performance[task_name][model_name] = value['summary_metrics']
        ...
        if task_performance:
            ...
            for task_name, model_metrics in task_performance.items():
                ...
                if metrics:
                    ...
                    for model_name, task_metrics in model_metrics.items():
                        if isinstance(task_metrics, dict):
                            values = [f"{task_metrics.get(metric, 0):.4f}" for metric in metrics]
        ...
        for model_name, model_results in results.items():
            for benchmark_name, benchmark_result in model_results.items():
                if hasattr(benchmark_result, 'results') and 'computational_metrics' in benchmark_result.results:
                    efficiency_data[model_name] = benchmark_result.results['computational_metrics']
        
        if efficiency_data:
            f.write("| Model | Avg Inference Time (s) | Peak Memory (MB) | Throughput (samples/s) |\n")
            f.write("|-------|----------------------|------------------|----------------------|\n")
            ...
            for model_name, metrics in efficiency_data.items():
                ...
                f.write(f"| {model_name} | {inf_time:.4f} | {memory:.1f} | {throughput:.1f} |\n")
        ...
        f.write("1. **Model Selection**: Choose model size based on task complexity and computational constraints\n")
        f.write("2. **Task Optimization**: Focus additional training on underperforming tasks\n")
        f.write("3. **Efficiency Optimization**: Consider model compression for deployment scenarios\n")
        f.write("4. **Scalability**: Monitor memory usage for long sequences in production\n")
        f.write("5. **Further Analysis**: Conduct task-specific hyperparameter optimization\n\n")
        
        f.write("---\n")
        f.write("*This report was generated automatically by the Hyena-GLT benchmarking framework.*\n")
