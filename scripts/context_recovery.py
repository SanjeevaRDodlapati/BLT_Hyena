...
class ContextRecovery:
    ...
    def check_git_status(self):
        ...
        # Last commit
        last_commit = self.run_command("git log -1 --pretty=format:'%h - %s (%cr) <%an>'")
        git_status['last_commit'] = last_commit
        
        # Commits since last tag
        commits_since_tag = self.run_command("git rev-list $(git describe --tags --abbrev=0)..HEAD --count")
        ...
        for dir_name in directories:
            ...
            if dir_path.exists():
                ...
                structure[dir_name] = {
                    ...
                    'total_files': len(list(dir_path.rglob('*'))) - len(list(dir_path.rglob('*/')))
                }
        ...
        if self.verbose:
            for dir_name, stats in structure.items():
                if isinstance(stats, dict) and 'total_files' in stats:
                    print(f"  {dir_name}: {stats['total_files']} files "
    ...
    def check_test_status(self):
        ...
        if tests_dir.exists():
            ...
            unit_tests = list((tests_dir / 'unit').rglob('test_*.py')) if (tests_dir / 'unit').exists() else []
            integration_tests = list((tests_dir / 'integration').rglob('test_*.py')) if (tests_dir / 'integration').exists() else []
    ...
    def generate_summary(self):
        ...
        if not self.report.get('project_structure', {}).get('PROJECT_STATE.md', {}).get('exists'):
