...
def create_sample_genomic_data(num_samples: int = 20, seq_length: int = 64) -> list:
    ...
    for i in range(num_samples):
        ...
        sequences.append({
            'sequence': sequence,
            'label': i % 3  # Simple classification labels
        })
...
def demo_data_processing(seq_length: int = 64, batch_size: int = 4):
    ...
    tokens = tokenizer.encode(test_sequence, max_length=seq_length, padding=True, truncation=True)
    ...
    print(f"   ✅ Dataset item shape: input_ids={sample_item['input_ids'].shape}, label={sample_item['label']}")
...
def demo_model_architecture(config: HyenaGLTConfig, seq_length: int = 64):
    ...
    if outputs['last_hidden_state'].shape[1] != input_ids.shape[1]:
        ...
        print(f"   ✅ BLT Token Merging: {input_ids.shape[1]} -> {outputs['last_hidden_state'].shape[1]} tokens ({compression_ratio:.1f}x compression)")
...
def run_complete_demo(seq_length: int = 64, batch_size: int = 4):
    ...
    print(f"✅ Forward pass: Successful with dynamic token compression")
    print(f"✅ Interpretability: Analysis framework operational")
