...
from hyena_glt.model.operators import HyenaOperator, DynamicConvolution, HyenaFilter, GenomicPositionalEncoding
...
def test_hyena_filter():
    ...
    assert filter_coeffs.shape == expected_shape, f"Expected {expected_shape}, got {filter_coeffs.shape}"
...
def test_genomic_positional_encoding():
    ...
    assert encoding.shape == expected_shape, f"Expected {expected_shape}, got {encoding.shape}"
...
def test_gradient_flow():
    ...
    assert any(p.grad is not None for p in hyena_op.parameters()), "No gradients for parameters"
