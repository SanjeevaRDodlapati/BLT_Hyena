...
class PerformanceBenchmark:
    ...
    def measure_latency(self, model: nn.Module, input_data: torch.Tensor) -> Dict[str, float]:
    ...
    def run_single_benchmark(self, seq_len: int, batch_size: int) -> Dict:
        ...
        results['relative'] = {
            'latency_ratio': blt_latency['avg_latency_ms'] / baseline_latency['avg_latency_ms'],
            'memory_ratio': blt_memory['cpu_peak_mb'] / baseline_memory['cpu_peak_mb'],
            'throughput_ratio': blt_latency['throughput_samples_per_sec'] / baseline_latency['throughput_samples_per_sec']
        }
        
        if torch.cuda.is_available():
            results['relative']['gpu_memory_ratio'] = blt_memory['gpu_peak_mb'] / baseline_memory['gpu_peak_mb']
    ...
    def analyze_results(self, results: List[Dict]):
        ...
        # Summary statistics
        print(f"\n📊 SUMMARY STATISTICS:")
        print(f"   Latency Ratio (BLT/Baseline): {np.mean(latency_ratios):.3f} ± {np.std(latency_ratios):.3f}")
        print(f"   Memory Ratio (BLT/Baseline):  {np.mean(memory_ratios):.3f} ± {np.std(memory_ratios):.3f}")
        print(f"   Throughput Ratio (BLT/Baseline): {np.mean(throughput_ratios):.3f} ± {np.std(throughput_ratios):.3f}")
        
        # Detailed results table
        print(f"\n📋 DETAILED RESULTS:")
        print(f"{'Seq Len':<8} {'Batch':<6} {'Latency Ratio':<14} {'Memory Ratio':<13} {'Throughput Ratio':<16}")
        print("-" * 70)
        
        for result in results:
            config = result['config']
            relative = result['relative']
            print(f"{config['seq_len']:<8} {config['batch_size']:<6} "
        ...
        if large_seq_results:
            large_seq_latency = np.mean([r['relative']['latency_ratio'] for r in large_seq_results])
