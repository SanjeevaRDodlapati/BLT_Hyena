...
def create_synthetic_data(batch_size: int, seq_len: int, vocab_size: int, device: torch.device):
...
def benchmark_distributed_training(
    ...
    if device_manager.is_main_process:
        ...
        logger.info(f"Device manager: rank={device_manager.rank}, local_rank={device_manager.local_rank}")
    ...
    if device_manager.is_main_process:
        logger.info(f"Starting benchmark: {num_steps} steps, batch_size={batch_size}, seq_len={seq_len}")
    ...
    for step in range(num_steps):
        ...
        # Create batch data
        batch = create_synthetic_data(batch_size, seq_len, vocab_size, device_manager.device)
        ...
        if device_manager.is_main_process and step % 10 == 0:
            logger.info(f"Step {step}/{num_steps}: loss={loss.item():.4f}, time={step_time:.3f}s")
    ...
    return {
        ...
        'tokens_per_second': tokens_per_second if device_manager.is_main_process else None,
...
def main():
    ...
    parser.add_argument('--use-fsdp', action='store_true', help='Use FSDP instead of DDP')
