...
from hyena_glt.training.finetuning import FineTuner, FinetuningConfig, TaskSpecificFineTuner
...
def create_dataset(data_path: str, task_type: str, tokenizer, max_length: int, label_names=None):
...
def main():
    ...
    parser.add_argument("--task_type", type=str, required=True,
                       choices=["sequence_classification", "token_classification", "sequence_generation"],
    ...
    parser.add_argument("--k", type=int, default=6,
                       help="K-mer size for DNA/RNA tokenizers")
    ...
    parser.add_argument("--use_layer_wise_decay", action="store_true",
                       help="Use layer-wise learning rate decay")
    parser.add_argument("--layer_wise_lr_decay", type=float, default=0.9,
    ...
    if args.num_labels is None and args.task_type in ["sequence_classification", "token_classification"]:
        if args.label_names:
            args.num_labels = len(args.label_names)
        else:
            raise ValueError(f"Must provide either --num_labels or --label_names for {args.task_type}")
