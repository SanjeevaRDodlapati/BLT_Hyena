...
def test_cluster_config(GPUClusterConfig):
    ...
    try:
        ...
        print(
            f"✓ Environment config: nodes={env_config.nodes}, gpus_per_node={env_config.gpus_per_node}"
        )
...
def test_backward_compatibility():
    ...
    try:
        ...
        torch.nn.Linear(10, 5).to(old_device)
...
def main():
    ...
    imports_ok, DeviceManager, GPUClusterConfig, init_distributed_training = (
        test_basic_imports()
    )
    ...
    if imports_ok and device_ok and config_ok and compat_ok and patterns_ok:
        ...
        print("\nDeployment Options:")
        print(
            "📋 Single Node: torchrun --nproc_per_node=4 scripts/benchmarks/benchmark_distributed.py"
        )
