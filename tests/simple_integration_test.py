...
def test_basic_integration():
    ...
    # Sample data
    sequences = ["ATCGATCGATCG", "GCTAGCTAGCTA", "ATGCATGCATGC", "CGATCGATCGAT"]
    labels = [0, 1, 0, 1]
    ...
    data = [
        ...
        for seq, label in zip(sequences, labels, strict=False)
    ]  # Use 'labels' for collator
    ...
    dataloader = GenomicDataLoader(
        dataset=dataset, batch_size=2, shuffle=False, tokenizer=tokenizer, max_length=32
    )
...
def test_convenience_functions():
    ...
    train_data = [
        {"sequence": seq, "labels": label}
        for seq, label in zip(train_sequences, train_labels, strict=False)
    ]
    val_data = [
        {"sequence": seq, "labels": label}
        for seq, label in zip(val_sequences, val_labels, strict=False)
    ]
...
def test_config_integration():
    ...
    custom_config = HyenaGLTConfig(
        vocab_size=1000, hidden_size=256, num_layers=6, max_position_embeddings=2048
    )
...
def test_tensor_operations():
    ...
    dataset = GenomicDataset(data=data, tokenizer=tokenizer, max_length=16)
