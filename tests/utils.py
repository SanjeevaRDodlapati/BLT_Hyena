...
class DataGenerator:
    ...
    def generate_dna_sequence(
        length: int, vocab_size: int = 4, seed: int | None = None
    ) -> torch.Tensor:
    ...
    def generate_protein_sequence(
        length: int, vocab_size: int = 20, seed: int | None = None
    ) -> torch.Tensor:
    ...
    @staticmethod
    def generate_batch(
        batch_size: int, seq_length: int, vocab_size: int, seed: int | None = None
    ) -> torch.Tensor:
    ...
    @staticmethod
    def generate_token_classification_data(
        batch_size: int,
        seq_length: int,
        vocab_size: int,
        num_classes: int,
    ...
    def generate_genomic_data(
        task: str, batch_size: int = 4, seq_length: int = 128, **kwargs
...
class ModelTestUtils:
    ...
    @staticmethod
    def create_test_model(config_dict: dict[str, Any] | None = None) -> HyenaGLT:
    ...
    def check_gradient_flow(
        model: torch.nn.Module, input_ids: torch.Tensor, labels: torch.Tensor = None
    ) -> dict[str, bool]:
