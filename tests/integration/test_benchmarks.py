...
from tests.utils import ModelTestUtils, PerformanceTestUtils, TestConfig
...
class TestMemoryBenchmarks:
    ...
    def test_gpu_memory_usage(self, memory_model):
        ...
        for i in range(1, len(memory_usage)):
            ...
            expected_ratio = (
                memory_usage[i]["batch_size"] / memory_usage[i - 1]["batch_size"]
            )
...
class TestScalabilityBenchmarks:
    ...
    def test_hyena_vs_attention_scaling(self):
        ...
        for i in range(1, len(hyena_times)):
            ...
            print(
                f"Seq len {sequence_lengths[i]}: {hyena_times[i]*1000:.2f}ms, "
...
class TestComparativeBenchmarks:
    ...
    def test_hyena_glt_vs_baselines(self):
        ...
        for i, seq_len in enumerate(sequence_lengths):
            ...
            print(
                f"Seq len {seq_len}: Hyena-GLT {hyena_time*1000:.2f}ms, "
    ...
    def test_different_model_architectures(self):
        ...
        for result in results:
            print(
                f"{result['name']}: {result['params']:,} params, "
...
class TestPerformanceRegression:
    ...
    def test_performance_baselines(self):
        ...
        print(
            f"Baseline performance: {avg_time*1000:.2f}ms (target: <{expected_max_time*1000:.2f}ms)"
        )
    ...
    def test_memory_baselines(self):
        ...
        assert (
            param_memory_mb < expected_max_memory
        ), f"Memory regression: {param_memory_mb:.2f}MB > {expected_max_memory}MB"

        print(
            f"Parameter memory: {param_memory_mb:.2f}MB (target: <{expected_max_memory}MB)"
        )
...
class TestBenchmarkVisualization:
    ...
    def test_memory_scaling_visualization(self, temp_dir):
        ...
        quadratic = [
            memory_usage[0] * (s / sequence_lengths[0]) ** 2 for s in sequence_lengths
        ]
