...
class TestInferenceBenchmarks:
    ...
    def test_sequence_classification_speed(self):
        ...
        for key, metrics in results.items():
            print(f"{key}: {metrics['sequences_per_second']:.2f} seq/s, "
    ...
    def test_memory_efficiency(self):
        ...
        for config_name, config_dict in configs:
            ...
            memory_results[config_name] = {
                ...
                'memory_efficiency': total_params / peak_memory_mb if peak_memory_mb > 0 else 0
            }
        
        # Print memory results
        print("\nMemory Efficiency Results:")
        for config_name, metrics in memory_results.items():
            print(f"{config_name}: {metrics['parameters']:,} params, "
                  f"{metrics['model_size_mb']:.1f}MB model, "
                  f"{metrics['peak_inference_memory_mb']:.1f}MB peak")
        
        # Basic assertions
        assert memory_results['small']['model_size_mb'] < memory_results['medium']['model_size_mb']
        assert memory_results['small']['parameters'] < memory_results['medium']['parameters']
    ...
    def test_gpu_utilization(self):
        ...
        for batch_size, metrics in utilization_results.items():
            print(f"Batch {batch_size}: {metrics['throughput']:.1f} seq/s, "
                  f"{metrics['memory_mb']:.0f}MB")
        
        # Throughput should generally increase with batch size
        # (up to memory limits)
        throughputs = [metrics['throughput'] for metrics in utilization_results.values()]
...
class TestScalabilityBenchmarks:
    ...
    def test_batch_size_scaling(self):
        ...
        for batch_size, metrics in batch_scaling_results.items():
            print(f"Batch {batch_size}: {metrics['sequences_per_second']:.1f} seq/s, "
                  f"efficiency {metrics['efficiency']:.3f}")
        
        # Throughput should generally increase with batch size
        throughputs = [metrics['sequences_per_second'] 
