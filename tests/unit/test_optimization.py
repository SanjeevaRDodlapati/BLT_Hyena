...
from hyena_glt.optimization.quantization import (
    QuantizationConfig, DynamicQuantizer, StaticQuantizer,
    QATQuantizer, QuantizationUtils
)
from hyena_glt.optimization.pruning import (
    PruningConfig, MagnitudePruner, StructuredPruner,
    GradientPruner, PruningScheduler
)
from hyena_glt.optimization.distillation import (
    DistillationConfig, KnowledgeDistiller, AttentionDistiller,
    FeatureDistiller, DistillationLoss
)
from hyena_glt.optimization.memory import (
    MemoryOptimizer, GradientCheckpointing, ActivationCheckpointing,
    MemoryProfiler, MemoryUtils
)
from hyena_glt.optimization.deployment import (
    ModelDeployer, TorchScriptConverter, ONNXConverter,
    OptimizationPipeline, DeploymentConfig
)
from hyena_glt.config import HyenaGLTConfig
from tests.utils import TestConfig, ModelTestUtils
...
class TestDynamicQuantizer:
    ...
    def test_quantization_accuracy(self, quantizer):
        ...
        relative_error = torch.abs(original_output - quantized_output) / (torch.abs(original_output) + 1e-8)
        assert relative_error.mean() < 0.5  # Allow 50% relative error due to quantization
...
class TestQATQuantizer:
    ...
    def test_prepare_qat_model(self, quantizer):
        ...
        has_fake_quant = any(
            "fake_quant" in str(type(module)).lower() 
...
class TestKnowledgeDistiller:
    ...
    def test_temperature_scaling(self, distiller):
        ...
        entropy_low = -(soft_probs_low * torch.log(soft_probs_low + 1e-8)).sum(-1).mean()
        entropy_high = -(soft_probs_high * torch.log(soft_probs_high + 1e-8)).sum(-1).mean()
...
class TestOptimizationPipeline:
    ...
    def test_pipeline_with_validation(self, pipeline):
        ...
        val_data = [(torch.randn(1, 10, 4), torch.randint(0, 4, (1, 10))) for _ in range(5)]
    ...
    def test_optimization_metrics(self, pipeline):
        ...
        metrics = pipeline.compute_optimization_metrics(
