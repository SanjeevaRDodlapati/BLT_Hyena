...
from hyena_glt.training.optimization import AdamWWithScheduler, create_optimizer, create_scheduler
from hyena_glt.training.curriculum import CurriculumLearning
from hyena_glt.training.multitask import MultiTaskLoss, TaskWeightScheduler, TaskConfig
...
class TestOptimization:
    ...
    def test_adamw_with_scheduler(self):
        ...
        batch = DataGenerator.generate_genomic_data("dna_classification", batch_size=2, seq_length=32)
...
class TestCheckpointing:
    ...
    def test_best_checkpoint_tracking(self, temp_dir):
        ...
        manager.save_checkpoint(
            model=model, optimizer=optimizer, epoch=1, step=100,
            metrics={'val_loss': 0.8}
        )
        manager.save_checkpoint(
            model=model, optimizer=optimizer, epoch=2, step=200,
            metrics={'val_loss': 0.5}  # Better
        )
        manager.save_checkpoint(
            model=model, optimizer=optimizer, epoch=3, step=300,
